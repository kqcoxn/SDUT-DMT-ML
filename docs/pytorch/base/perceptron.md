...
$$\ell(y, \boldsymbol{x}, \boldsymbol{w}) = \max(0, -y \langle \boldsymbol{w}, \boldsymbol{x} \rangle) = \max(0, -y \boldsymbol{w}^T \boldsymbol{x})$$
...
\begin{cases}
- y \cdot \boldsymbol{x}, & \text{if } y \boldsymbol{w}^T \boldsymbol{x} < 0 \\
0, & \text{otherwise}
\end{cases}
$$
...
设数据在特征空间能被半径为 $r$ 的圆（球）覆盖，并且分类时有余量（即 $\sigma$ 函数的输入不会取使输出模棱两可的值）$y(\boldsymbol{x}^T \boldsymbol{w}) \geq \rho$，若初始参数满足 $|\boldsymbol{w}|^2 + b^2 \leq 1$，则感知机保证在 $\frac{r^2 + 1}{\rho^2}$ 步内收敛。

## 线性模型的缺陷

在前面的课程中我们学习了 softmax 回归，线性回归，它们有将输入向量与一个权重向量做内积再与一个偏置相加得到一个值的过程：
...
![](../images/base/多层分类XOR1.png)

![](../images/base/多层分类XOR2.png)

这实际上将信息进行了多层次的传递：

![](../images/base/XOR信息多层次传递.png)
...
ReLU 函数通过将相应的活性值设为 0，仅保留正元素并丢弃所有负元素。为了直观感受一下，我们可以画出函数的曲线图。正如从图中所看到，激活函数是分段线性的。使用 ReLU 的原因是，它求导表现得特别好：要么让参数消失，要么让参数通过。这使得优化表现得更好，并且 ReLU 减轻了困扰以往神经网络的梯度消失问题。
